...
ziroomIndex.init = function(){
	...
	ziroomIndex.SearchList('#i_q_keyword_1','.t_ajaxbox','#searchListBtn','/hz/list/suggest');
    ...
    $(window).resize(function(){
        ...
        $('#foucsSlideList,#foucsSlideList li,#foucsSlideList li img,.foucsSlide').css({'width':w+'px'});
        $('#foucsSlideList,#foucsSlideList li,#foucsSlideList li img,.foucsSlide').css({'height':h+'px'});
    });
    var w = $(window).width();
...
ziroomIndex.SearchList = function(input,box,button,url){
	var step = -1;
    var oldValue="";
    var listTimer=null;
    var oInput = $(input),
...
$(function() {
   ...
   ziroomIndex.init(); 
   //indexCms();
    jQuery(".foucsSlide").slide({mainCell:".bd ul",effect:"fold",autoPlay:true,interTime:5000});
    ...
    var oSlideLeftBar = $('#slideLeftBar');
       ...
       $('.bigtit').each(function(){
        ...
        function setActive(winTop){
            ...
            //document.title = winTop +'a='+$('#tit_a').offset().top+'b='+$('#tit_b').offset().top;
            if(winTop >640){
                oSlideLeftBar.show().css({'position':'fixed','top':'10px'});
                
            }else{
                oSlideLeftBar.hide().css({'position':'absolute','top':'640px'});
